...
OUT_CSV  = os.path.join(OPEC_DIR, "opec_texts_raw.csv")
...
def parse_month_year_from_filename(fname: str):
    """Try to get month/year from filename like 'MOMR_October_2025.pdf' or 'OPEC_MOMR_Sep_2021.pdf'."""
    name = os.path.splitext(os.path.basename(fname))[0]
    m = MONTH_PATTERN.search(name)
    y = YEAR_PATTERN.search(name)
    if m and y:
        month_txt = m.group(1).lower()
        month_num = MONTH_MAP.get(month_txt, None)
        year = int(y.group(1))
...
def main():
    ...
    for pdf in tqdm(pdfs):
        fpath = os.path.join(OPEC_DIR, pdf)
        ...
        # detect month/year (filename first, fallback to text)
        ...
        # build report_date (YYYY-MM-01) if possible
        report_date = None
        month_name = None
        ...
        records.append({
            "filename": pdf,
            "report_date": report_date,     # e.g., 2025-11-01
            "year": year,                   # e.g., 2025
            "month_num": month_num,         # e.g., 11
    ...
    if missing_dates:
        print(f"  {missing_dates} file(s) missing month/year. You can fix names or we’ll handle later in cleaning.")
    else:
